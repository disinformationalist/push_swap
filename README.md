...
### But first the basic stats.

## Much Fast, Such Speed
### Specifically 100 and 500 numbers are required by the problem as given. Here are the speeds for this solution.


| Integers to sort | Average moves required | Lowest moves required |
|------------------|------------------------|-----------------------|
| 100              | 530                    | 430                   |
| 500              | 3900                   | 3500                  |


## The Problem, The Goal, and Rules
...
- ### For full credit, sort 100 integers in under 700, and 500 integers in under 5500.
...
**NOTE** : Each stack is set up as a doubly linked list.
...
**Step 1. Get final locations**

We figure out where the values are supposed to go by sorting the data directly with quicksort,
and then we place the final location for a value with its node. For example if we have the values:
...
of the stack and moving them to B, each time finding the value that requires the least number 
of moves(rotations or reverse rotations) to bring to the top of stack A and push it to B and 
sending that one next.

Then we do it for the next third of the original value, 500 / 3 = 166, so the next 166 go.
...
the next 42 because 84 / 2 = 42 and then the next 24 because 84 / 3 = 24. We repeat this recursion until 
less than 10 remain in stack A. Why break it up like this? Numerous schemes were tried for this recursion and
this provided the best result.
...
rotate here, we also check if the next value coming from A is going to be rotated up(is in the lower half)
and is also not already in top position, and if so we save a move by using the double rotation.

Next we sort those 9 still in A, ending up with them in A using an optimized method for sorting 9
without disturbing stack B. We also can perform checks for cylic sorted, and typically use this with small stacks
to sort them quickly if we are in luck and only need to rotate.

Now we have moved things to B, minimized moves used while acheiving the maximum presorting. Time to 
bring it back.
...
1. Find their 'target node' in A. This is 
...
When looking for the cheapest route we must examine 6 different ways to possible bring the nodes together.





Next we compare the return cost of all the nodes in B, and find the cheapest one,
This is the one we will return using its fastest route.
